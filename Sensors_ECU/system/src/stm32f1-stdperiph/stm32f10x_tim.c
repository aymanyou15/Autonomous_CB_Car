...
static void TI4_Config(TIM_TypeDef* TIMx, uint16_t TIM_ICPolarity, uint16_t TIM_ICSelection,
                       uint16_t TIM_ICFilter);
static void (*TIM_ptrfUpdateNotif)(void);
/**
  * @}
  */
...
void TIM_RegisterCbfOnUpdate(void (*Add_pfUpdateNotif)(void))
{
	if(Add_pfUpdateNotif != NULL)
	{
		TIM_ptrfUpdateNotif = Add_pfUpdateNotif;
	}
}
...
{
	//1ms passed
	if(TIM_GetITStatus((TIM_TypeDef*)TIM2_BASE,TIM_IT_Update))
	{
		TIM_ClearFlag((TIM_TypeDef*)TIM2_BASE,TIM_FLAG_Update);
		if(TIM_ptrfUpdateNotif != NULL)
		{
			TIM_ptrfUpdateNotif();
		}
	}
